Admin_Brands = new Vue({
    ...
    computed: {
        computedSlug() {
            return this.generateSlug(this.NameBrands);
        }
    },
    watch: {
        computedSlug(newVal) {
    ...
    methods: {
        ...
        loadSubItems() {
            ...
            if ($.fn.DataTable.isDataTable('#Brands_table')) {
                currentPage = $('#Brands_table').DataTable().page();
            ...
            axios.get("/Products/GetAllBrands")
                ...
                .then(() => {
                    const table = $("#Brands_table").DataTable({
                        'columnDefs': [{
                            'targets': [-1],
                            'orderable': false,
                        }],
        ...
        async addCategory() {
            try {
                ...
                formData.append('NameBrand', this.NameBrands);
                formData.append('PrPath', this.$refs.PrPath.files[0]);
                formData.append('Slug', this.Slug);

                await axios.post('/Products/AddPrBrand', formData,
                    {
                        headers: {
        ...
        getItemsById(id) {
            axios.get(`/Products/getIdPrBrand?id=${id}`)
                .then((response) => {
                    this.id = response.data.id;
                    this.NameBrands = response.data.nameBrand;
                    this.Slug = response.data.slug;
                    this.imageSubCategory = response.data.imgBrand;
                    return Promise.resolve();
                });
        },
        resetData() {
        ...
        async editCategory() {
            try {
                ...
                await axios.post('/Products/UpdatePrBrand', formData,
                    {
                        headers: {
                            'Content-Type': 'multipart/form-data'
                        }
        ...
        getItemsByIdDelete(id) {
            axios.get(`/Products/getIdPrBrand/${id}`)
                .then((response) => {
                    this.id = response.data.id;
                    if (this.id != null) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                const formData = new FormData();
                                formData.append('ID', this.id);
                                axios.post('/Products/DeletePrBrand', formData, {
                                    headers: {
