Admin_subclb = new Vue({
    ...
    methods: {
        ...
        loadSubItems() {
            let currentPage = 0;
            if ($.fn.DataTable.isDataTable('#subclb_table')) {
                currentPage = $('#subclb_table').DataTable().page();
                $('#subclb_table').DataTable().destroy();
            }
            axios.get("/Products/GetAllSubCLB")
                .then((response) => {
                    this.dataSubItems = response.data;
                    return Promise.resolve();
                })
                .then(() => {
                    const table = $("#subclb_table").DataTable({
                        'columnDefs': [{
                            'targets': [-1],
                            'orderable': false,
                        }],
        ...
        async addCategory() {
            try {
                ...
                formData.append('NameCLB', this.SubClbName);
                formData.append('IDMain', this.CategoryID);
                formData.append('PrPath', this.$refs.PrPath.files[0]);
                formData.append('Slug', this.Slug);


                await axios.post('/Products/AddSubPrClb', formData,
                    {
                        headers: {
        ...
        getItemsById(id) {
            axios.get(`/Products/getIdSubCLB?id=${id}`)
                .then((response) => {
                    this.id = response.data.id;
                    this.SubClbName = response.data.nameCLB;
                    this.Slug = response.data.slug;
                    this.CategoryID = response.data.idMain;
                    this.imageSubCategory = response.data.imgCLB;
                    return Promise.resolve();
                });
        },
        resetData() {
        ...
        async editCategory() {
            try {
                ...
                await axios.post('/Products/UpdateSubClb', formData,
                    {
                        headers: {
                            'Content-Type': 'multipart/form-data'
                        }
        ...
        getItemsByIdDelete(id) {
            axios.get(`/Products/getIdCLB/${id}`)
                .then((response) => {
                    this.id = response.data.id;
                    if (this.id != null) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                const formData = new FormData();
                                formData.append('ID', this.id);
                                axios.post('/Products/DeleteCLB', formData, {
                                    headers: {
