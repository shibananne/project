Admin_CLB = new Vue({
    ...
    computed: {
        computedSlug() {
            return this.generateSlug(this.NameCLB);
        }
    },
    watch: {
        computedSlug(newVal) {
    ...
    methods: {
        ...
        loadSubItems() {
            let currentPage = 0;
            if ($.fn.DataTable.isDataTable('#CLB_table')) {
                currentPage = $('#CLB_table').DataTable().page();
                $('#CLB_table').DataTable().destroy();
            }
            axios.get("/Products/GetAllCLB")
                ...
                .then(() => {
                    const table =  $("#CLB_table").DataTable({
                        'columnDefs': [{
                            'targets': [-1],
                            'orderable': false,
                        }],
        ...
        async addCategory() {
            try {
                ...
                formData.append('NameMainCLB', this.NameCLB);
                formData.append('Slug', this.Slug);

                await axios.post('/Products/AddCLB', formData,
                    {
                        headers: {
        ...
        getItemsById(id) {
            axios.get(`/Products/getIdCLB?id=${id}`)
                .then((response) => {
                    this.id = response.data.id;
                    this.NameCLB = response.data.nameMainCLB;
        ...
        async editCategory() {
            try {
                ...
                await axios.post('/Products/UpdateCLB', formData,
                    {
                        headers: {
                            'Content-Type': 'multipart/form-data'
                        }
