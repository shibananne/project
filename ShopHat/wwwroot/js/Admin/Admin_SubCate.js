Admin_subcategory = new Vue({
    el: '#Admin_subcategory',
    data: {
        dataItems: [],
        dataSubItems: [],
        id: "",
    ...
    computed: {
        computedSlug() {
            return this.generateSlug(this.SubCategoryName);
        }
    },
    watch: {
        computedSlug(newVal) {
    ...
    methods: {
        ...
        loadSubItems() {
            let currentPage = 0;
            if ($.fn.DataTable.isDataTable('#subcate_table')) {
                currentPage = $('#subcate_table').DataTable().page();
                $('#subcate_table').DataTable().destroy();
            }
            axios.get("/Products/GetAllSubCategory")
                ...
                .then(() => {
                    const table =  $("#subcate_table").DataTable({
                        'columnDefs': [{
                            'targets': [-1],
                            'orderable': false,
                        }],
        ...
        getItemsById(id) {
            axios.get(`/Products/getIdSubMenu?id=${id}`)
                .then((response) => {

                    this.id = response.data.id;
                    this.CategoryID = response.data.cateId;
                    this.CategoryName = response.data.nameCate;
                    this.SubCategoryName = response.data.nameSub;
                    this.imageSubCategory = response.data.imgSubCate;
        ...
        async editCategory() {
            try {
                ...
                await axios.post('/Products/UpdateSubMenu', formData,
                    {
                        headers: {
                            'Content-Type': 'multipart/form-data'
                        }
        ...
        getItemsByIdDelete(id) {
            axios.get(`/Products/getIdSubMenu/${id}`)
                .then((response) => {
                    this.id = response.data.id;
                    if (this.id != null) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                const formData = new FormData();
                                formData.append('ID', this.id);
                                axios.post('/Products/DeleteSubMenu', formData, {
                                    headers: {
